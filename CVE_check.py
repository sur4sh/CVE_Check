...
def CVEnew(datas):
    ...
    #print("the Lister is {}".format(data3))
    #print("DATA IS {}".format(data2))
     tester3=[]
     for elem in data2:
        ...
        for i in range(len(data2)):
            for j in range(len(datas)):
                if str(datas[j]) in data2[i]:
                  ...
                  source="CVE_NEW"
                  tester=re.findall(r'CVE-\d+-\d+',data2[i])
                  #print("tester is:{}".format(tester))
                  tester1=re.findall(r'(?<=\d{4}\s).*',data2[i])
                  #print("tester1 is:{}".format(tester1))
                  datass="\bSoftware:{}\nCve:{}\nDescription:{}".format(datas[j],tester[0],tester1[0])
...
def ibmx(datas):
#data=pandas.read_csv("software_export_99_840855 (2).csv")
#datas=data["Software"].tolist()
 headers_dict = {"Accept": "application/json","Authorization":"Basic {ibmx auth token}"}
 ...
 for i in range(length):
    for j in range(len(datas)):
     #print("FOR {} IS {}".format(i,y[i]["title"]))
     #if str(datas[j]) in y[i]["title"]:
     if str(datas[j]) in y[i]["platforms_affected"][0]:
        #print(y[i]["description"],y[i]["stdcode"])
...
def CVE_STALKER(datas):
 ...
 tester3=[]
 strings = "No New Vulnerability Found"
 #print("data:{} and length:{}".format(dff,len(dff)))
 for j in range(len(lister)):
  for z in range(len(datas)):
     # test=re.findall(r'XSS',str(j))
     if str(datas[z]) in str(lister[j]):
         ...
         mycursor = mydb.cursor()
         sql = "INSERT INTO CVE2 (Date,source,software,cve,description) VALUES (%s,%s,%s, %s, %s)"
         source="CVE_STALKER"
...
if __name__ == "__main__":
    data=pandas.read_csv("software.csv"). 
